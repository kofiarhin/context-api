...
function createService(options = {}) {
  ...
  return Object.freeze({
    ...
    async listTeams(input = {}) {
      return list(
        await client.request('GET', '/v2/teams', { query: input }),
        'teams',
        serializer.team
      );
    },
    async getTeam({ teamId }) {
      if (env.vercelTeamId && teamId !== env.vercelTeamId)
        policy.assertProjectAllowed('__forbidden_team__');
      return serializer.team(
        await client.request('GET', `/v2/teams/${encode(teamId)}`, { query: { teamId: undefined } })
      );
    },
    ...
    async updateProject({ project, approval, ...changes }) {
      ...
      return serializer.project(
        await client.request('PATCH', `/v9/projects/${encode(project)}`, { body: changes })
      );
    },
    ...
    async pauseProject({ project, approval }) {
      ...
      policy.requireProductionApproval(approval, `project ${project}`);
      return serializer.project(
        await client.request('POST', `/v1/projects/${encode(project)}/pause`)
      );
    },
    async unpauseProject({ project, approval }) {
      ...
      policy.requireProductionApproval(approval, `project ${project}`);
      return serializer.project(
        await client.request('POST', `/v1/projects/${encode(project)}/unpause`)
      );
    },
    ...
    async getDeployment({ deployment }) {
      return serializer.deployment(
        await client.request('GET', `/v13/deployments/${encode(deployment)}`)
      );
    },
    async createDeployment(input) {
      ...
      if (target === 'production') {
        policy.requireProductionApproval(
          input.approval,
          `deployment for ${input.project || input.name}`
        );
      }
    ...
    async cancelDeployment({ deployment }) {
      return serializer.deployment(
        await client.request('PATCH', `/v12/deployments/${encode(deployment)}/cancel`)
      );
    },
    ...
    async getDeploymentEvents({ deployment, ...query }) {
      const payload = await client.request('GET', `/v3/deployments/${encode(deployment)}/events`, {
        query,
      });
    ...
    async listDeploymentFiles({ deployment }) {
      ...
      return {
        data: (payload.files || payload || []).map((file) =>
          serializer.compact({ name: file.name, type: file.type, uid: file.uid, size: file.size })
        ),
        meta: {},
      };
    },
    async promoteDeployment({ project, deployment, approval }) {
      ...
      return serializer.compact(
        await client.request(
          'POST',
          `/v10/projects/${encode(project)}/promote/${encode(deployment)}`
        )
      );
    },
    async rollbackProject({ project, deployment, approval }) {
      ...
      return serializer.compact(
        await client.request(
          'POST',
          `/v10/projects/${encode(project)}/rollback/${encode(deployment)}`
        )
      );
    },
    ...
    async createEnvironmentVariable({ project, approval, ...input }) {
      ...
      if ((input.target || []).includes('production'))
        policy.requireProductionApproval(approval, `environment variable ${input.key}`);
      return serializer.environmentVariable(
        await client.request('POST', `/v10/projects/${encode(project)}/env`, { body: input })
      );
    },
    async updateEnvironmentVariable({ project, variable, approval, ...input }) {
      ...
      if ((input.target || []).includes('production'))
        policy.requireProductionApproval(approval, `environment variable ${variable}`);
      return serializer.environmentVariable(
        await client.request('PATCH', `/v9/projects/${encode(project)}/env/${encode(variable)}`, {
          body: input,
        })
      );
    },
    ...
    async getProjectDomain({ project, domain }) {
      ...
      return serializer.domain(
        await client.request('GET', `/v9/projects/${encode(project)}/domains/${encode(domain)}`)
      );
    },
    async addProjectDomain({ project, domain, approval, ...input }) {
      ...
      return serializer.domain(
        await client.request('POST', `/v10/projects/${encode(project)}/domains`, {
          body: { name: domain, ...input },
        })
      );
    },
    async verifyProjectDomain({ project, domain, approval }) {
      ...
      return serializer.domain(
        await client.request(
          'POST',
          `/v9/projects/${encode(project)}/domains/${encode(domain)}/verify`
        )
      );
    },
    ...
    async assignAlias({ deployment, alias, approval }) {
      ...
      return serializer.alias(
        await client.request('POST', `/v2/deployments/${encode(deployment)}/aliases`, {
          body: { alias },
        })
      );
    },
    ...
    async getDomainConfig({ domain }) {
      policy.assertDomainAllowed(domain);
      return serializer.compact(
        await client.request('GET', `/v6/domains/${encode(domain)}/config`)
      );
    },
    async listDnsRecords({ domain, ...query }) {
      policy.assertDomainAllowed(domain);
      const payload = await client.request('GET', `/v4/domains/${encode(domain)}/records`, {
        query,
      });
    ...
    async createDnsRecord({ domain, approval, ...input }) {
      ...
      return serializer.dnsRecord(
        await client.request('POST', `/v2/domains/${encode(domain)}/records`, { body: input })
      );
    },
    async updateDnsRecord({ domain, record, approval, ...input }) {
      ...
      return serializer.dnsRecord(
        await client.request('PATCH', `/v1/domains/records/${encode(record)}`, { body: input })
      );
    },
