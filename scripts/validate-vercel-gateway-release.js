...
if (fs.existsSync(schemaPath)) {
  ...
  if (!schema.includes('bearerAuth')) fail('Action schema must declare bearer authentication');
  if (/decrypted value/i.test(schema)) fail('Action schema must not expose decrypted-value operations');

  const ids = [...schema.matchAll(/^\s+operationId:\s+(\S+)\s*$/gm)].map((match) => match[1]);
  if (new Set(ids).size !== ids.length) fail('operation IDs must be unique');
  for (const id of expectedOperations) {
    if (!ids.includes(id)) fail(`missing operation ID ${id}`);
  }
}
...
if (fs.existsSync(servicePath)) {
  ...
  if (/decrypt/i.test(service)) fail('service must not include decrypted environment-variable reads');
  if (!service.includes("body.target = 'preview'")) fail('deployment creation must default to Preview');
  if (!service.includes('requireProductionApproval')) fail('production approval policy is not enforced');
  if (!service.includes('requireDestructiveConfirmation')) fail('destructive confirmation policy is not enforced');
}

if (failed) process.exitCode = 1;
else process.stdout.write('Vercel gateway release validation passed.\n');
